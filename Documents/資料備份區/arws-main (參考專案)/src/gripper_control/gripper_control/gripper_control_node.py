...
class OnRobotGripperRG2(object):
    ...
    def close_gripper(self, target_width=-8, target_force=40, payload=0.5, set_payload=False, depth_compensation=False, slave=False, wait=2):
        urscript = OnRobotGripperRG2Script()
        urscript._rg2_command(target_width, target_force, payload, set_payload, depth_compensation, slave)
        self.robot.send_program(urscript())
        time.sleep(4)
...
class GripperNode(Node):
    def __init__(self):
        ...
        self.gripper.open_gripper(
            target_width=50,  # Width in mm, 100 is fully open
    ...
    def listener_callback(self, msg):
        ...
        if msg.data == "open":
            self.gripper.open_gripper(target_width=100)
        elif msg.data == "close":
            self.gripper.close_gripper(target_width=-8)
        else:
