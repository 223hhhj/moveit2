...
class GripperNode (Node):
    def __init__ (self):
        ...
        self.subscription = self.create_subscription(  #訂閱custom_gripper的topic
            Int32MultiArray,
            'custom_gripper',
    ...
    def listener_callback(self, msg):           #將topic裡面的a,b放到open gripper讓夾爪做動

        a, b = msg.data[:2]                          
        self.gripper.open_gripper(a,b)
        self.get_logger().info("[INFO]: gripper move")

        self.minimal_client.send_request()  #讓external control(end effector的動作)繼續做動，調用下面的class
...
#-------- 讓external control(end effector的動作)繼續做動
