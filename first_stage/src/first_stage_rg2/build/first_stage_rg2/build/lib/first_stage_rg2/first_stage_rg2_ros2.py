...
class OnRobotGripperRG2(object):
    def __init__(self, robot):
        self.robot = robot
        self.gripping_state = "OPEN"
        self.last_gripping_state = "OPEN"
    ...
    def get_current_state(self):
        try:
            ...
            digital_value = self.robot.get_digital_in(0)
            
            if digital_value:
                self.gripping_state = "TRUE"
            else:
                self.gripping_state = "FALSE"

            if self.gripping_state != self.last_gripping_state:
                self.last_gripping_state = self.gripping_state
...
class GripperNode(Node):
    def __init__(self):
        ...
        # New publisher for gripper state
        self.state_publisher = self.create_publisher(String, 'gripper_state', 10)
        self.timer = self.create_timer(1.0, self.publish_state)

    def listener_callback(self, msg):
        a, b = msg.data[:2]
        self.gripper.open_gripper(a, b)
        self.get_logger().info("[INFO]: gripper move")
        self.minimal_client.send_request()

    def publish_state(self):
        state = self.gripper.get_current_state()
        if state is not None:
            msg = String()
            msg.data = f"State: {state}"
            self.state_publisher.publish(msg)
            self.get_logger().info(msg.data)
        else:
            self.get_logger().warn('Unable to get current state')
